...
Blockly.Dart.init = function(workspace) {
  ...
  for (var i = 0; i < devVarList.length; i++) {
    defvars.push(Blockly.Dart.variableDB_.getName(devVarList[i], Blockly.Names.DEVELOPER_VARIABLE_TYPE));
  }
  var variables = Blockly.Variables.allUsedVarModels(workspace);
  for (var i = 0; i < variables.length; i++) {
    defvars.push(Blockly.Dart.variableDB_.getName(variables[i].getId(), Blockly.Variables.NAME_TYPE));
  }
  if (defvars.length) {
    Blockly.Dart.definitions_["variables"] = "var " + defvars.join(", ") + ";";
  }
};
...
Blockly.Dart.getAdjusted = function(block, atId, opt_delta, opt_negate, opt_order) {
  ...
  if (delta) {
    var at = Blockly.Dart.valueToCode(block, atId, Blockly.Dart.ORDER_ADDITIVE) || defaultAtIndex;
  } else {
    if (opt_negate) {
      var at = Blockly.Dart.valueToCode(block, atId, Blockly.Dart.ORDER_UNARY_PREFIX) || defaultAtIndex;
    } else {
      var at = Blockly.Dart.valueToCode(block, atId, order) || defaultAtIndex;
    }
  }
...
Blockly.Dart["colour_rgb"] = function(block) {
  ...
  Blockly.Dart.definitions_["import_dart_math"] = "import 'dart:math' as Math;";
  var functionName = Blockly.Dart.provideFunction_("colour_rgb", ["String " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(num r, num g, num b) {", "  num rn = (Math.max(Math.min(r, 100), 0) * 2.55).round();", "  String rs = rn.toInt().toRadixString(16);", "  rs = '0$rs';", "  rs = rs.substring(rs.length - 2);", "  num gn = (Math.max(Math.min(g, 100), 0) * 2.55).round();", "  String gs = gn.toInt().toRadixString(16);", "  gs = '0$gs';", "  gs = gs.substring(gs.length - 2);", "  num bn = (Math.max(Math.min(b, 100), 0) * 2.55).round();", 
  "  String bs = bn.toInt().toRadixString(16);", "  bs = '0$bs';", "  bs = bs.substring(bs.length - 2);", "  return '#$rs$gs$bs';", "}"]);
...
Blockly.Dart["colour_blend"] = function(block) {
  ...
  var ratio = Blockly.Dart.valueToCode(block, "RATIO", Blockly.Dart.ORDER_NONE) || 0.5;
  Blockly.Dart.definitions_["import_dart_math"] = "import 'dart:math' as Math;";
  var functionName = Blockly.Dart.provideFunction_("colour_blend", ["String " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(String c1, String c2, num ratio) {", "  ratio = Math.max(Math.min(ratio, 1), 0);", "  int r1 = int.parse('0x${c1.substring(1, 3)}');", "  int g1 = int.parse('0x${c1.substring(3, 5)}');", "  int b1 = int.parse('0x${c1.substring(5, 7)}');", "  int r2 = int.parse('0x${c2.substring(1, 3)}');", "  int g2 = int.parse('0x${c2.substring(3, 5)}');", "  int b2 = int.parse('0x${c2.substring(5, 7)}');", 
  "  num rn = (r1 * (1 - ratio) + r2 * ratio).round();", "  String rs = rn.toInt().toRadixString(16);", "  num gn = (g1 * (1 - ratio) + g2 * ratio).round();", "  String gs = gn.toInt().toRadixString(16);", "  num bn = (b1 * (1 - ratio) + b2 * ratio).round();", "  String bs = bn.toInt().toRadixString(16);", "  rs = '0$rs';", "  rs = rs.substring(rs.length - 2);", "  gs = '0$gs';", "  gs = gs.substring(gs.length - 2);", "  bs = '0$bs';", "  bs = bs.substring(bs.length - 2);", "  return '#$rs$gs$bs';", 
  "}"]);
...
Blockly.Dart["lists_create_with"] = function(block) {
  ...
  for (var i = 0; i < block.itemCount_; i++) {
    elements[i] = Blockly.Dart.valueToCode(block, "ADD" + i, Blockly.Dart.ORDER_NONE) || "null";
  }
    ...
    switch(where) {
      ...
      case "RANDOM":
        ...
        if (mode == "REMOVE") {
          ...
          if (mode == "GET") {
            var functionName = Blockly.Dart.provideFunction_("lists_get_random_item", ["dynamic " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List my_list) {", "  int x = new Math.Random().nextInt(my_list.length);", "  return my_list[x];", "}"]);
...
Blockly.Dart["lists_sort"] = function(block) {
  ...
  var sortFunctionName = Blockly.Dart.provideFunction_("lists_sort", ["List " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List list, String type, int direction) {", "  var compareFuncs = {", '    "NUMERIC": (a, b) => (direction * a.compareTo(b)).toInt(),', '    "TEXT": (a, b) => direction * ' + "a.toString().compareTo(b.toString()),", '    "IGNORE_CASE": ', "       (a, b) => direction * ", "      a.toString().toLowerCase().compareTo(b.toString().toLowerCase())", "  };", "  list = new List.from(list);", 
  "  var compare = compareFuncs[type];", "  list.sort(compare);", "  return list;", "}"]);
...
Blockly.Dart["controls_repeat_ext"] = function(block) {
  if (block.getField("TIMES")) {
    ...
    var repeats = Blockly.Dart.valueToCode(block, "TIMES", Blockly.Dart.ORDER_ASSIGNMENT) || "0";
  }
  ...
  if (!repeats.match(/^\w+$/) && !Blockly.isNumber(repeats)) {
    var endVar = Blockly.Dart.variableDB_.getDistinctName("repeat_end", Blockly.Variables.NAME_TYPE);
...
Blockly.Dart["controls_for"] = function(block) {
  ...
  if (Blockly.isNumber(argument0) && Blockly.isNumber(argument1) && Blockly.isNumber(increment)) {
    ...
    if (!argument0.match(/^\w+$/) && !Blockly.isNumber(argument0)) {
      var startVar = Blockly.Dart.variableDB_.getDistinctName(variable0 + "_start", Blockly.Variables.NAME_TYPE);
...
Blockly.Dart["math_number"] = function(block) {
  ...
  if (code == Infinity) {
    ...
    if (code == -Infinity) {
      ...
      order = code < 0 ? Blockly.Dart.ORDER_UNARY_PREFIX : Blockly.Dart.ORDER_ATOMIC;
    }
  }
  return [code, order];
};
Blockly.Dart["math_arithmetic"] = function(block) {
  var OPERATORS = {"ADD":[" + ", Blockly.Dart.ORDER_ADDITIVE], "MINUS":[" - ", Blockly.Dart.ORDER_ADDITIVE], "MULTIPLY":[" * ", Blockly.Dart.ORDER_MULTIPLICATIVE], "DIVIDE":[" / ", Blockly.Dart.ORDER_MULTIPLICATIVE], "POWER":[null, Blockly.Dart.ORDER_NONE]};
...
Blockly.Dart["math_single"] = function(block) {
  ...
  if (operator == "ABS" || operator.substring(0, 5) == "ROUND") {
    arg = Blockly.Dart.valueToCode(block, "NUM", Blockly.Dart.ORDER_UNARY_POSTFIX) || "0";
  } else {
    if (operator == "SIN" || operator == "COS" || operator == "TAN") {
      arg = Blockly.Dart.valueToCode(block, "NUM", Blockly.Dart.ORDER_MULTIPLICATIVE) || "0";
    } else {
      arg = Blockly.Dart.valueToCode(block, "NUM", Blockly.Dart.ORDER_NONE) || "0";
    }
  }
  switch(operator) {
    case "ABS":
...
Blockly.Dart["math_constant"] = function(block) {
  var CONSTANTS = {"PI":["Math.pi", Blockly.Dart.ORDER_UNARY_POSTFIX], "E":["Math.e", Blockly.Dart.ORDER_UNARY_POSTFIX], "GOLDEN_RATIO":["(1 + Math.sqrt(5)) / 2", Blockly.Dart.ORDER_MULTIPLICATIVE], "SQRT2":["Math.sqrt2", Blockly.Dart.ORDER_UNARY_POSTFIX], "SQRT1_2":["Math.sqrt1_2", Blockly.Dart.ORDER_UNARY_POSTFIX], "INFINITY":["double.infinity", Blockly.Dart.ORDER_ATOMIC]};
...
Blockly.Dart["math_random_int"] = function(block) {
  ...
  var functionName = Blockly.Dart.provideFunction_("math_random_int", ["int " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(num a, num b) {", "  if (a > b) {", "    // Swap a and b to ensure a is smaller.", "    num c = a;", "    a = b;", "    b = c;", "  }", "  return new Math.Random().nextInt(b - a + 1) + a;", "}"]);
...
Blockly.Dart["procedures_defreturn"] = function(block) {
  ...
  if (Blockly.Dart.INFINITE_LOOP_TRAP) {
    branch = Blockly.Dart.INFINITE_LOOP_TRAP.replace(/%1/g, "'" + block.id + "'") + branch;
  }
  ...
  for (var i = 0; i < block.arguments_.length; i++) {
    args[i] = Blockly.Dart.variableDB_.getName(block.arguments_[i], Blockly.Variables.NAME_TYPE);
  }
...
Blockly.Dart["procedures_callreturn"] = function(block) {
  ...
  var args = [];
  for (var i = 0; i < block.arguments_.length; i++) {
    args[i] = Blockly.Dart.valueToCode(block, "ARG" + i, Blockly.Dart.ORDER_NONE) || "null";
  }
...
Blockly.Dart["text_join"] = function(block) {
  switch(block.itemCount_) {
    ...
    default:
      var elements = new Array(block.itemCount_);
      for (var i = 0; i < block.itemCount_; i++) {
        elements[i] = Blockly.Dart.valueToCode(block, "ADD" + i, Blockly.Dart.ORDER_NONE) || "''";
      }
...
Blockly.Dart["text_charAt"] = function(block) {
  ...
  switch(where) {
    ...
    case "FROM_END":
      var at = Blockly.Dart.getAdjusted(block, "AT", 1);
      var functionName = Blockly.Dart.provideFunction_("text_get_from_end", ["String " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(String text, num x) {", "  return text[text.length - x];", "}"]);
      code = functionName + "(" + text + ", " + at + ")";
      return [code, Blockly.Dart.ORDER_UNARY_POSTFIX];
    case "RANDOM":
      Blockly.Dart.definitions_["import_dart_math"] = "import 'dart:math' as Math;";
      var functionName = Blockly.Dart.provideFunction_("text_random_letter", ["String " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(String text) {", "  int x = new Math.Random().nextInt(text.length);", "  return text[x];", "}"]);
...
Blockly.Dart["text_getSubstring"] = function(block) {
  ...
  if (where1 == "FIRST" && where2 == "LAST") {
    ...
    if (text.match(/^'?\w+'?$/) || where1 != "FROM_END" && where2 == "FROM_START") {
      switch(where1) {
        case "FROM_START":
          var at1 = Blockly.Dart.getAdjusted(block, "AT1");
          break;
...
Blockly.Dart["text_changeCase"] = function(block) {
  var OPERATORS = {"UPPERCASE":".toUpperCase()", "LOWERCASE":".toLowerCase()", "TITLECASE":null};
  ...
  if (operator) {
    ...
    var functionName = Blockly.Dart.provideFunction_("text_toTitleCase", ["String " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(String str) {", "  RegExp exp = new RegExp(r'\\b');", "  List<String> list = str.split(exp);", "  final title = new StringBuffer();", "  for (String part in list) {", "    if (part.length > 0) {", "      title.write(part[0].toUpperCase());", "      if (part.length > 0) {", "        title.write(part.substring(1).toLowerCase());", "      }", "    }", "  }", "  return title.toString();", 
    "}"]);
...
Blockly.Dart["text_trim"] = function(block) {
  var OPERATORS = {"LEFT":".replaceFirst(new RegExp(r'^\\s+'), '')", "RIGHT":".replaceFirst(new RegExp(r'\\s+$'), '')", "BOTH":".trim()"};
...
Blockly.Dart["text_count"] = function(block) {
  ...
  var functionName = Blockly.Dart.provideFunction_("text_count", ["int " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(String haystack, String needle) {", "  if (needle.length == 0) {", "    return haystack.length + 1;", "  }", "  int index = 0;", "  int count = 0;", "  while (index != -1) {", "    index = haystack.indexOf(needle, index);", "    if (index != -1) {", "      count++;", "     index += needle.length;", "    }", "  }", "  return count;", "}"]);
