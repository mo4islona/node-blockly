...
Blockly.JavaScript.getAdjusted = function(block, atId, opt_delta, opt_negate, opt_order) {
  ...
  if (delta > 0) {
    var at = Blockly.JavaScript.valueToCode(block, atId, Blockly.JavaScript.ORDER_ADDITION) || defaultAtIndex;
  } else {
    if (delta < 0) {
      var at = Blockly.JavaScript.valueToCode(block, atId, Blockly.JavaScript.ORDER_SUBTRACTION) || defaultAtIndex;
    } else {
      if (opt_negate) {
        var at = Blockly.JavaScript.valueToCode(block, atId, Blockly.JavaScript.ORDER_UNARY_NEGATION) || defaultAtIndex;
      } else {
        var at = Blockly.JavaScript.valueToCode(block, atId, order) || defaultAtIndex;
      }
    }
  }
...
Blockly.JavaScript["colour_blend"] = function(block) {
  ...
  var ratio = Blockly.JavaScript.valueToCode(block, "RATIO", Blockly.JavaScript.ORDER_COMMA) || 0.5;
  var functionName = Blockly.JavaScript.provideFunction_("colourBlend", ["function " + Blockly.JavaScript.FUNCTION_NAME_PLACEHOLDER_ + "(c1, c2, ratio) {", "  ratio = Math.max(Math.min(Number(ratio), 1), 0);", "  var r1 = parseInt(c1.substring(1, 3), 16);", "  var g1 = parseInt(c1.substring(3, 5), 16);", "  var b1 = parseInt(c1.substring(5, 7), 16);", "  var r2 = parseInt(c2.substring(1, 3), 16);", "  var g2 = parseInt(c2.substring(3, 5), 16);", "  var b2 = parseInt(c2.substring(5, 7), 16);", "  var r = Math.round(r1 * (1 - ratio) + r2 * ratio);", 
  "  var g = Math.round(g1 * (1 - ratio) + g2 * ratio);", "  var b = Math.round(b1 * (1 - ratio) + b2 * ratio);", "  r = ('0' + (r || 0).toString(16)).slice(-2);", "  g = ('0' + (g || 0).toString(16)).slice(-2);", "  b = ('0' + (b || 0).toString(16)).slice(-2);", "  return '#' + r + g + b;", "}"]);
...
Blockly.JavaScript["lists_getIndex"] = function(block) {
  ...
  switch(where) {
    ...
    case "FROM_END":
      ...
      break;
    case "RANDOM":
      var functionName = Blockly.JavaScript.provideFunction_("listsGetRandomItem", ["function " + Blockly.JavaScript.FUNCTION_NAME_PLACEHOLDER_ + "(list, remove) {", "  var x = Math.floor(Math.random() * list.length);", "  if (remove) {", "    return list.splice(x, 1)[0];", "  } else {", "    return list[x];", "  }", "}"]);
    ...
    if (list.match(/^\w+$/) || where1 != "FROM_END" && where2 == "FROM_START") {
      switch(where1) {
        case "FROM_START":
          ...
          break;
        case "FROM_END":
          var at1 = Blockly.JavaScript.getAdjusted(block, "AT1", 1, false, Blockly.JavaScript.ORDER_SUBTRACTION);
          at1 = list + ".length - " + at1;
          break;
        case "FIRST":
          var at1 = "0";
...
Blockly.JavaScript["lists_sort"] = function(block) {
  ...
  var getCompareFunctionName = Blockly.JavaScript.provideFunction_("listsGetSortCompare", ["function " + Blockly.JavaScript.FUNCTION_NAME_PLACEHOLDER_ + "(type, direction) {", "  var compareFuncs = {", '    "NUMERIC": function(a, b) {', "        return parseFloat(a) - parseFloat(b); },", '    "TEXT": function(a, b) {', "        return a.toString() > b.toString() ? 1 : -1; },", '    "IGNORE_CASE": function(a, b) {', "        return a.toString().toLowerCase() > " + "b.toString().toLowerCase() ? 1 : -1; },", 
...
Blockly.JavaScript["math_arithmetic"] = function(block) {
  var OPERATORS = {"ADD":[" + ", Blockly.JavaScript.ORDER_ADDITION], "MINUS":[" - ", Blockly.JavaScript.ORDER_SUBTRACTION], "MULTIPLY":[" * ", Blockly.JavaScript.ORDER_MULTIPLICATION], "DIVIDE":[" / ", Blockly.JavaScript.ORDER_DIVISION], "POWER":[null, Blockly.JavaScript.ORDER_COMMA]};
...
Blockly.JavaScript["math_constant"] = function(block) {
  var CONSTANTS = {"PI":["Math.PI", Blockly.JavaScript.ORDER_MEMBER], "E":["Math.E", Blockly.JavaScript.ORDER_MEMBER], "GOLDEN_RATIO":["(1 + Math.sqrt(5)) / 2", Blockly.JavaScript.ORDER_DIVISION], "SQRT2":["Math.SQRT2", Blockly.JavaScript.ORDER_MEMBER], "SQRT1_2":["Math.SQRT1_2", Blockly.JavaScript.ORDER_MEMBER], "INFINITY":["Infinity", Blockly.JavaScript.ORDER_ATOMIC]};
...
Blockly.JavaScript["math_number_property"] = function(block) {
  ...
  if (dropdown_property == "PRIME") {
    var functionName = Blockly.JavaScript.provideFunction_("mathIsPrime", ["function " + Blockly.JavaScript.FUNCTION_NAME_PLACEHOLDER_ + "(n) {", "  // https://en.wikipedia.org/wiki/Primality_test#Naive_methods", "  if (n == 2 || n == 3) {", "    return true;", "  }", "  // False if n is NaN, negative, is 1, or not whole.", "  // And false if n is divisible by 2 or 3.", "  if (isNaN(n) || n <= 1 || n % 1 != 0 || n % 2 == 0 ||" + " n % 3 == 0) {", "    return false;", "  }", "  // Check all the numbers of form 6k +/- 1, up to sqrt(n).", 
...
Blockly.JavaScript["math_on_list"] = function(block) {
  ...
  switch(func) {
    ...
    case "MODE":
      ...
      break;
    case "STD_DEV":
      var functionName = Blockly.JavaScript.provideFunction_("mathStandardDeviation", ["function " + Blockly.JavaScript.FUNCTION_NAME_PLACEHOLDER_ + "(numbers) {", "  var n = numbers.length;", "  if (!n) return null;", "  var mean = numbers.reduce(function(x, y) {return x + y;}) / n;", "  var variance = 0;", "  for (var j = 0; j < n; j++) {", "    variance += Math.pow(numbers[j] - mean, 2);", "  }", "  variance = variance / n;", "  return Math.sqrt(variance);", "}"]);
      list = Blockly.JavaScript.valueToCode(block, "LIST", Blockly.JavaScript.ORDER_NONE) || "[]";
      code = functionName + "(" + list + ")";
      break;
    case "RANDOM":
      var functionName = Blockly.JavaScript.provideFunction_("mathRandomList", ["function " + Blockly.JavaScript.FUNCTION_NAME_PLACEHOLDER_ + "(list) {", "  var x = Math.floor(Math.random() * list.length);", "  return list[x];", "}"]);
      list = Blockly.JavaScript.valueToCode(block, "LIST", Blockly.JavaScript.ORDER_NONE) || "[]";
      code = functionName + "(" + list + ")";
      break;
    default:
...
Blockly.JavaScript["text_join"] = function(block) {
  switch(block.itemCount_) {
    ...
    default:
      var elements = new Array(block.itemCount_);
      for (var i = 0; i < block.itemCount_; i++) {
        elements[i] = Blockly.JavaScript.valueToCode(block, "ADD" + i, Blockly.JavaScript.ORDER_COMMA) || "''";
      }
...
Blockly.JavaScript["text_getSubstring"] = function(block) {
  ...
  if (where1 == "FIRST" && where2 == "LAST") {
    ...
    if (text.match(/^'?\w+'?$/) || where1 != "FROM_END" && where1 != "LAST" && where2 != "FROM_END" && where2 != "LAST") {
      switch(where1) {
        case "FROM_START":
          var at1 = Blockly.JavaScript.getAdjusted(block, "AT1");
          break;
...
Blockly.JavaScript["text_changeCase"] = function(block) {
  ...
  if (operator) {
    ...
    var functionName = Blockly.JavaScript.provideFunction_("textToTitleCase", ["function " + Blockly.JavaScript.FUNCTION_NAME_PLACEHOLDER_ + "(str) {", "  return str.replace(/\\S+/g,", "      function(txt) {return txt[0].toUpperCase() + " + "txt.substring(1).toLowerCase();});", "}"]);
    var code = functionName + "(" + text + ")";
  }
  return [code, Blockly.JavaScript.ORDER_FUNCTION_CALL];
};
Blockly.JavaScript["text_trim"] = function(block) {
  var OPERATORS = {"LEFT":".replace(/^[\\s\\xa0]+/, '')", "RIGHT":".replace(/[\\s\\xa0]+$/, '')", "BOTH":".trim()"};
...
Blockly.JavaScript["text_replace"] = function(block) {
  ...
  var functionName = Blockly.JavaScript.provideFunction_("textReplace", ["function " + Blockly.JavaScript.FUNCTION_NAME_PLACEHOLDER_ + "(haystack, needle, replacement) {", "  needle = " + 'needle.replace(/([-()\\[\\]{}+?*.$\\^|,:#<!\\\\])/g,"\\\\$1")', '                 .replace(/\\x08/g,"\\\\x08");', "  return haystack.replace(new RegExp(needle, 'g'), replacement);", "}"]);
