...
Blockly.Python.init = function(workspace) {
  ...
  for (var i = 0; i < devVarList.length; i++) {
    defvars.push(Blockly.Python.variableDB_.getName(devVarList[i], Blockly.Names.DEVELOPER_VARIABLE_TYPE) + " = None");
  }
  var variables = Blockly.Variables.allUsedVarModels(workspace);
  for (var i = 0; i < variables.length; i++) {
    defvars.push(Blockly.Python.variableDB_.getName(variables[i].getId(), Blockly.Variables.NAME_TYPE) + " = None");
  }
...
Blockly.Python["lists_indexOf"] = function(block) {
  ...
  if (block.getFieldValue("END") == "FIRST") {
    var functionName = Blockly.Python.provideFunction_("first_index", ["def " + Blockly.Python.FUNCTION_NAME_PLACEHOLDER_ + "(my_list, elem):", "  try: index = my_list.index(elem)" + firstIndexAdjustment, "  except: index =" + errorIndex, "  return index"]);
    var code = functionName + "(" + list + ", " + item + ")";
    return [code, Blockly.Python.ORDER_FUNCTION_CALL];
  }
  var functionName = Blockly.Python.provideFunction_("last_index", ["def " + Blockly.Python.FUNCTION_NAME_PLACEHOLDER_ + "(my_list, elem):", "  try: index = len(my_list) - my_list[::-1].index(elem)" + lastIndexAdjustment, "  except: index =" + errorIndex, "  return index"]);
  var code = functionName + "(" + list + ", " + item + ")";
  return [code, Blockly.Python.ORDER_FUNCTION_CALL];
};
  ...
  switch(where) {
    ...
    case "RANDOM":
      ...
      if (mode == "GET") {
        ...
        var functionName = Blockly.Python.provideFunction_("lists_remove_random_item", ["def " + Blockly.Python.FUNCTION_NAME_PLACEHOLDER_ + "(myList):", "  x = int(random.random() * len(myList))", "  return myList.pop(x)"]);
    ...
    var scrub = function(arg, suffix) {
      if (Blockly.isNumber(arg)) {
        ...
        if (arg.match(/^\w+$/)) {
          ...
          var varName = Blockly.Python.variableDB_.getDistinctName(variable0 + suffix, Blockly.Variables.NAME_TYPE);
...
Blockly.Python["math_arithmetic"] = function(block) {
  var OPERATORS = {"ADD":[" + ", Blockly.Python.ORDER_ADDITIVE], "MINUS":[" - ", Blockly.Python.ORDER_ADDITIVE], "MULTIPLY":[" * ", Blockly.Python.ORDER_MULTIPLICATIVE], "DIVIDE":[" / ", Blockly.Python.ORDER_MULTIPLICATIVE], "POWER":[" ** ", Blockly.Python.ORDER_EXPONENTIATION]};
...
Blockly.Python["math_constant"] = function(block) {
  var CONSTANTS = {"PI":["math.pi", Blockly.Python.ORDER_MEMBER], "E":["math.e", Blockly.Python.ORDER_MEMBER], "GOLDEN_RATIO":["(1 + math.sqrt(5)) / 2", Blockly.Python.ORDER_MULTIPLICATIVE], "SQRT2":["math.sqrt(2)", Blockly.Python.ORDER_MEMBER], "SQRT1_2":["math.sqrt(1.0 / 2)", Blockly.Python.ORDER_MEMBER], "INFINITY":["float('inf')", Blockly.Python.ORDER_ATOMIC]};
...
Blockly.Python["math_number_property"] = function(block) {
  ...
  if (dropdown_property == "PRIME") {
    ...
    var functionName = Blockly.Python.provideFunction_("math_isPrime", ["def " + Blockly.Python.FUNCTION_NAME_PLACEHOLDER_ + "(n):", "  # https://en.wikipedia.org/wiki/Primality_test#Naive_methods", "  # If n is not a number but a string, try parsing it.", "  if not isinstance(n, Number):", "    try:", "      n = float(n)", "    except:", "      return False", "  if n == 2 or n == 3:", "    return True", "  # False if n is negative, is 1, or not whole," + " or if n is divisible by 2 or 3.", "  if n <= 1 or n % 1 != 0 or n % 2 == 0 or n % 3 == 0:", 
    "    return False", "  # Check all the numbers of form 6k +/- 1, up to sqrt(n).", "  for x in range(6, int(math.sqrt(n)) + 2, 6):", "    if n % (x - 1) == 0 or n % (x + 1) == 0:", "      return False", "  return True"]);
...
Blockly.Python["math_on_list"] = function(block) {
  ...
  switch(func) {
    ...
    case "AVERAGE":
      Blockly.Python.definitions_["from_numbers_import_Number"] = "from numbers import Number";
      var functionName = Blockly.Python.provideFunction_("math_mean", ["def " + Blockly.Python.FUNCTION_NAME_PLACEHOLDER_ + "(myList):", "  localList = [e for e in myList if isinstance(e, Number)]", "  if not localList: return", "  return float(sum(localList)) / len(localList)"]);
      code = functionName + "(" + list + ")";
      break;
    case "MEDIAN":
      Blockly.Python.definitions_["from_numbers_import_Number"] = "from numbers import Number";
      var functionName = Blockly.Python.provideFunction_("math_median", ["def " + Blockly.Python.FUNCTION_NAME_PLACEHOLDER_ + "(myList):", "  localList = sorted([e for e in myList if isinstance(e, Number)])", "  if not localList: return", "  if len(localList) % 2 == 0:", "    return (localList[len(localList) // 2 - 1] + " + "localList[len(localList) // 2]) / 2.0", "  else:", "    return localList[(len(localList) - 1) // 2]"]);
      code = functionName + "(" + list + ")";
      break;
    case "MODE":
      var functionName = Blockly.Python.provideFunction_("math_modes", ["def " + Blockly.Python.FUNCTION_NAME_PLACEHOLDER_ + "(some_list):", "  modes = []", "  # Using a lists of [item, count] to keep count rather than dict", '  # to avoid "unhashable" errors when the counted item is ' + "itself a list or dict.", "  counts = []", "  maxCount = 1", "  for item in some_list:", "    found = False", "    for count in counts:", "      if count[0] == item:", "        count[1] += 1", "        maxCount = max(maxCount, count[1])", 
      "        found = True", "    if not found:", "      counts.append([item, 1])", "  for counted_item, item_count in counts:", "    if item_count == maxCount:", "      modes.append(counted_item)", "  return modes"]);
      code = functionName + "(" + list + ")";
      break;
    case "STD_DEV":
      Blockly.Python.definitions_["import_math"] = "import math";
...
Blockly.Python["procedures_defreturn"] = function(block) {
  ...
  var devVarList = Blockly.Variables.allDeveloperVariables(workspace);
  for (var i = 0; i < devVarList.length; i++) {
    globals.push(Blockly.Python.variableDB_.getName(devVarList[i], Blockly.Names.DEVELOPER_VARIABLE_TYPE));
  }
...
Blockly.Python["text_join"] = function(block) {
  switch(block.itemCount_) {
    ...
    case 2:
      ...
      var code = Blockly.Python.text.forceString_(element0) + " + " + Blockly.Python.text.forceString_(element1);
...
Blockly.Python["text_changeCase"] = function(block) {
  var OPERATORS = {"UPPERCASE":".upper()", "LOWERCASE":".lower()", "TITLECASE":".title()"};
...
Blockly.Python["text_trim"] = function(block) {
  var OPERATORS = {"LEFT":".lstrip()", "RIGHT":".rstrip()", "BOTH":".strip()"};
