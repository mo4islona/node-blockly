...
Blockly.Lua["colour_rgb"] = function(block) {
  ...
  var g = Blockly.Lua.valueToCode(block, "GREEN", Blockly.Lua.ORDER_NONE) || 0;
  var b = Blockly.Lua.valueToCode(block, "BLUE", Blockly.Lua.ORDER_NONE) || 0;
  var code = functionName + "(" + r + ", " + g + ", " + b + ")";
  return [code, Blockly.Lua.ORDER_HIGH];
};
Blockly.Lua["colour_blend"] = function(block) {
  var functionName = Blockly.Lua.provideFunction_("colour_blend", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(colour1, colour2, ratio)", "  local r1 = tonumber(string.sub(colour1, 2, 3), 16)", "  local r2 = tonumber(string.sub(colour2, 2, 3), 16)", "  local g1 = tonumber(string.sub(colour1, 4, 5), 16)", "  local g2 = tonumber(string.sub(colour2, 4, 5), 16)", "  local b1 = tonumber(string.sub(colour1, 6, 7), 16)", "  local b2 = tonumber(string.sub(colour2, 6, 7), 16)", "  local ratio = math.min(1, math.max(0, ratio))", 
  "  local r = math.floor(r1 * (1 - ratio) + r2 * ratio + .5)", "  local g = math.floor(g1 * (1 - ratio) + g2 * ratio + .5)", "  local b = math.floor(b1 * (1 - ratio) + b2 * ratio + .5)", '  return string.format("#%02x%02x%02x", r, g, b)', "end"]);
  ...
  var ratio = Blockly.Lua.valueToCode(block, "RATIO", Blockly.Lua.ORDER_NONE) || 0;
...
Blockly.Lua["lists_create_with"] = function(block) {
  ...
  for (var i = 0; i < block.itemCount_; i++) {
    elements[i] = Blockly.Lua.valueToCode(block, "ADD" + i, Blockly.Lua.ORDER_NONE) || "None";
  }
...
Blockly.Lua["lists_indexOf"] = function(block) {
  ...
  if (block.getFieldValue("END") == "FIRST") {
    var functionName = Blockly.Lua.provideFunction_("first_index", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(t, elem)", "  for k, v in ipairs(t) do", "    if v == elem then", "      return k", "    end", "  end", "  return 0", "end"]);
  } else {
    var functionName = Blockly.Lua.provideFunction_("last_index", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(t, elem)", "  for i = #t, 1, -1 do", "    if t[i] == elem then", "      return i", "    end", "  end", "  return 0", "end"]);
  }
...
Blockly.Lua["lists_getIndex"] = function(block) {
  ...
  if ((where == "LAST" || where == "FROM_END" || where == "RANDOM") && !list.match(/^\w+$/)) {
    if (mode == "REMOVE") {
      ...
      var listVar = Blockly.Lua.variableDB_.getDistinctName("tmp_list", Blockly.Variables.NAME_TYPE);
      ...
      if (mode == "GET") {
        var functionName = Blockly.Lua.provideFunction_("list_get_" + where.toLowerCase(), ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(t" + (where == "FROM_END" || where == "FROM_START" ? ", at)" : ")"), "  return t[" + getIndex_("t", where, "at") + "]", "end"]);
      } else {
        var functionName = Blockly.Lua.provideFunction_("list_remove_" + where.toLowerCase(), ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(t" + (where == "FROM_END" || where == "FROM_START" ? ", at)" : ")"), "  return table.remove(t, " + getIndex_("t", where, "at") + ")", "end"]);
      }
...
Blockly.Lua["lists_setIndex"] = function(block) {
  ...
  if (mode == "SET") {
    ...
    code += "table.insert(" + list + ", " + (getIndex_(list, where, at) + (where == "LAST" ? " + 1" : "")) + ", " + value + ")";
  }
  return code + "\n";
};
Blockly.Lua["lists_getSublist"] = function(block) {
  ...
  var getIndex_ = Blockly.Lua.lists.getIndex_;
  var functionName = Blockly.Lua.provideFunction_("list_sublist_" + where1.toLowerCase() + "_" + where2.toLowerCase(), ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(source" + (where1 == "FROM_END" || where1 == "FROM_START" ? ", at1" : "") + (where2 == "FROM_END" || where2 == "FROM_START" ? ", at2" : "") + ")", "  local t = {}", "  local start = " + getIndex_("source", where1, "at1"), "  local finish = " + getIndex_("source", where2, "at2"), "  for i = start, finish do", "    table.insert(t, source[i])", 
  "  end", "  return t", "end"]);
...
Blockly.Lua["lists_sort"] = function(block) {
  ...
  var functionName = Blockly.Lua.provideFunction_("list_sort", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(list, typev, direction)", "  local t = {}", "  for n,v in pairs(list) do table.insert(t, v) end", "  local compareFuncs = {", "    NUMERIC = function(a, b)", "      return (tonumber(tostring(a)) or 0)", "          < (tonumber(tostring(b)) or 0) end,", "    TEXT = function(a, b)", "      return tostring(a) < tostring(b) end,", "    IGNORE_CASE = function(a, b)", "      return string.lower(tostring(a)) < string.lower(tostring(b)) end", 
  "  }", "  local compareTemp = compareFuncs[typev]", "  local compare = compareTemp", "  if direction == -1", "  then compare = function(a, b) return compareTemp(b, a) end", "  end", "  table.sort(t, compare)", "  return t", "end"]);
  var code = functionName + "(" + list + ',"' + type + '", ' + direction + ")";
  return [code, Blockly.Lua.ORDER_HIGH];
};
Blockly.Lua["lists_split"] = function(block) {
  ...
  if (mode == "SPLIT") {
    ...
    functionName = Blockly.Lua.provideFunction_("list_string_split", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(input, delim)", "  local t = {}", "  local pos = 1", "  while true do", "    next_delim = string.find(input, delim, pos)", "    if next_delim == nil then", "      table.insert(t, string.sub(input, pos))", "      break", "    else", "      table.insert(t, string.sub(input, pos, next_delim-1))", "      pos = next_delim + #delim", "    end", "  end", "  return t", "end"]);
  } else {
    if (mode == "JOIN") {
      if (!input) {
        input = "{}";
...
Blockly.Lua["math_arithmetic"] = function(block) {
  var OPERATORS = {ADD:[" + ", Blockly.Lua.ORDER_ADDITIVE], MINUS:[" - ", Blockly.Lua.ORDER_ADDITIVE], MULTIPLY:[" * ", Blockly.Lua.ORDER_MULTIPLICATIVE], DIVIDE:[" / ", Blockly.Lua.ORDER_MULTIPLICATIVE], POWER:[" ^ ", Blockly.Lua.ORDER_EXPONENTIATION]};
...
Blockly.Lua["math_constant"] = function(block) {
  var CONSTANTS = {PI:["math.pi", Blockly.Lua.ORDER_HIGH], E:["math.exp(1)", Blockly.Lua.ORDER_HIGH], GOLDEN_RATIO:["(1 + math.sqrt(5)) / 2", Blockly.Lua.ORDER_MULTIPLICATIVE], SQRT2:["math.sqrt(2)", Blockly.Lua.ORDER_HIGH], SQRT1_2:["math.sqrt(1 / 2)", Blockly.Lua.ORDER_HIGH], INFINITY:["math.huge", Blockly.Lua.ORDER_HIGH]};
...
Blockly.Lua["math_number_property"] = function(block) {
  ...
  if (dropdown_property == "PRIME") {
    var functionName = Blockly.Lua.provideFunction_("math_isPrime", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(n)", "  -- https://en.wikipedia.org/wiki/Primality_test#Naive_methods", "  if n == 2 or n == 3 then", "    return true", "  end", "  -- False if n is NaN, negative, is 1, or not whole.", "  -- And false if n is divisible by 2 or 3.", "  if not(n > 1) or n % 1 ~= 0 or n % 2 == 0 or n % 3 == 0 then", "    return false", "  end", "  -- Check all the numbers of form 6k +/- 1, up to sqrt(n).", 
    "  for x = 6, math.sqrt(n) + 1.5, 6 do", "    if n % (x - 1) == 0 or n % (x + 1) == 0 then", "      return false", "    end", "  end", "  return true", "end"]);
...
Blockly.Lua["math_on_list"] = function(block) {
  ...
  switch(func) {
    case "SUM":
      ...
      break;
    case "MIN":
      functionName = Blockly.Lua.provideFunction_("math_min", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(t)", "  if #t == 0 then", "    return 0", "  end", "  local result = math.huge", "  for _, v in ipairs(t) do", "    if v < result then", "      result = v", "    end", "  end", "  return result", "end"]);
      break;
    case "AVERAGE":
      ...
      break;
    case "MAX":
      functionName = Blockly.Lua.provideFunction_("math_max", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(t)", "  if #t == 0 then", "    return 0", "  end", "  local result = -math.huge", "  for _, v in ipairs(t) do", "    if v > result then", "      result = v", "    end", "  end", "  return result", "end"]);
      break;
    case "MEDIAN":
      functionName = Blockly.Lua.provideFunction_("math_median", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(t)", "  -- Source: http://lua-users.org/wiki/SimpleStats", "  if #t == 0 then", "    return 0", "  end", "  local temp={}", "  for _, v in ipairs(t) do", '    if type(v) == "number" then', "      table.insert(temp, v)", "    end", "  end", "  table.sort(temp)", "  if #temp % 2 == 0 then", "    return (temp[#temp/2] + temp[(#temp/2)+1]) / 2", "  else", "    return temp[math.ceil(#temp/2)]", 
      "  end", "end"]);
      break;
    case "MODE":
      functionName = Blockly.Lua.provideFunction_("math_modes", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(t)", "  -- Source: http://lua-users.org/wiki/SimpleStats", "  local counts={}", "  for _, v in ipairs(t) do", "    if counts[v] == nil then", "      counts[v] = 1", "    else", "      counts[v] = counts[v] + 1", "    end", "  end", "  local biggestCount = 0", "  for _, v  in pairs(counts) do", "    if v > biggestCount then", "      biggestCount = v", "    end", "  end", "  local temp={}", 
      "  for k, v in pairs(counts) do", "    if v == biggestCount then", "      table.insert(temp, k)", "    end", "  end", "  return temp", "end"]);
      break;
    case "STD_DEV":
      functionName = Blockly.Lua.provideFunction_("math_standard_deviation", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(t)", "  local m", "  local vm", "  local total = 0", "  local count = 0", "  local result", "  m = #t == 0 and 0 or " + provideSum() + "(t) / #t", "  for _, v in ipairs(t) do", "    if type(v) == 'number' then", "      vm = v - m", "      total = total + (vm * vm)", "      count = count + 1", "    end", "  end", "  result = math.sqrt(total / (count-1))", "  return result", 
      "end"]);
      break;
    case "RANDOM":
      functionName = Blockly.Lua.provideFunction_("math_random_list", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(t)", "  if #t == 0 then", "    return nil", "  end", "  return t[math.random(#t)]", "end"]);
      break;
    default:
...
Blockly.Lua["procedures_defreturn"] = function(block) {
  ...
  if (Blockly.Lua.INFINITE_LOOP_TRAP) {
    branch = Blockly.Lua.INFINITE_LOOP_TRAP.replace(/%1/g, "'" + block.id + "'") + branch;
  }
  ...
  for (var i = 0; i < block.arguments_.length; i++) {
    args[i] = Blockly.Lua.variableDB_.getName(block.arguments_[i], Blockly.Variables.NAME_TYPE);
  }
  ...
  if (block.itemCount_ == 0) {
    ...
    if (block.itemCount_ == 1) {
      ...
      if (block.itemCount_ == 2) {
        ...
        var elements = [];
        for (var i = 0; i < block.itemCount_; i++) {
          elements[i] = Blockly.Lua.valueToCode(block, "ADD" + i, Blockly.Lua.ORDER_NONE) || "''";
        }
...
Blockly.Lua["text_indexOf"] = function(block) {
  ...
  if (block.getFieldValue("END") == "FIRST") {
    var functionName = Blockly.Lua.provideFunction_("firstIndexOf", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(str, substr) ", "  local i = string.find(str, substr, 1, true)", "  if i == nil then", "    return 0", "  else", "    return i", "  end", "end"]);
  } else {
    var functionName = Blockly.Lua.provideFunction_("lastIndexOf", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(str, substr)", "  local i = string.find(string.reverse(str), " + "string.reverse(substr), 1, true)", "  if i then", "    return #str + 2 - i - #substr", "  end", "  return 0", "end"]);
  }
...
Blockly.Lua["text_charAt"] = function(block) {
  ...
  var code;
  if (where == "RANDOM") {
    var functionName = Blockly.Lua.provideFunction_("text_random_letter", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(str)", "  local index = math.random(string.len(str))", "  return string.sub(str, index, index)", "end"]);
    ...
    if (start.match(/^-?\w*$/)) {
      ...
      var functionName = Blockly.Lua.provideFunction_("text_char_at", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(str, index)", "  return string.sub(str, index, index)", "end"]);
  ...
  if (operator == "UPPERCASE") {
    ...
    if (operator == "LOWERCASE") {
      ...
      if (operator == "TITLECASE") {
        var functionName = Blockly.Lua.provideFunction_("text_titlecase", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(str)", "  local buf = {}", "  local inWord = false", "  for i = 1, #str do", "    local c = string.sub(str, i, i)", "    if inWord then", "      table.insert(buf, string.lower(c))", '      if string.find(c, "%s") then', "        inWord = false", "      end", "    else", "      table.insert(buf, string.upper(c))", "      inWord = true", "    end", "  end", "  return table.concat(buf)", 
        "end"]);
      }
    }
  }
...
Blockly.Lua["text_trim"] = function(block) {
  var OPERATORS = {LEFT:"^%s*(,-)", RIGHT:"(.-)%s*$", BOTH:"^%s*(.-)%s*$"};
...
Blockly.Lua["text_count"] = function(block) {
  ...
  var functionName = Blockly.Lua.provideFunction_("text_count", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(haystack, needle)", "  if #needle == 0 then", "    return #haystack + 1", "  end", "  local i = 1", "  local count = 0", "  while true do", "    i = string.find(haystack, needle, i, true)", "    if i == nil then", "      break", "    end", "    count = count + 1", "    i = i + #needle", "  end", "  return count", "end"]);
...
Blockly.Lua["text_replace"] = function(block) {
  ...
  var functionName = Blockly.Lua.provideFunction_("text_replace", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(haystack, needle, replacement)", "  local buf = {}", "  local i = 1", "  while i <= #haystack do", "    if string.sub(haystack, i, i + #needle - 1) == needle then", "      for j = 1, #replacement do", "        table.insert(buf, string.sub(replacement, j, j))", "      end", "      i = i + #needle", "    else", "      table.insert(buf, string.sub(haystack, i, i))", "      i = i + 1", 
  "    end", "  end", "  return table.concat(buf)", "end"]);
